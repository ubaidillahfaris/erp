...
class JournalBalanceTest extends TestCase
{
    ...
    {
        // Entry 1
        $j1 = Journal::factory()->create(['date' => '2023-01-01', 'type' => 'debit', 'amount' => 1000]);
        $this->assertEquals(1000, $j1->fresh()->balance);

        // Entry 2
        $j2 = Journal::factory()->create(['date' => '2023-01-02', 'type' => 'kredit', 'amount' => 300]);
        $this->assertEquals(700, $j2->fresh()->balance);

        // Entry 3 (Newest)
        $j3 = Journal::factory()->create(['date' => '2023-01-03', 'type' => 'debit', 'amount' => 500]);
    ...
    public function test_backfill_logic(): void
    {
        ...
        Journal::create(['date' => '2023-01-01', 'type' => 'debit', 'amount' => 1000, 'category' => 'test', 'payment_method' => 'cash', 'balance' => 0]);
        Journal::create(['date' => '2023-01-02', 'type' => 'kredit', 'amount' => 200, 'category' => 'test', 'payment_method' => 'cash', 'balance' => 0]);
        Journal::create(['date' => '2023-01-03', 'type' => 'debit', 'amount' => 300, 'category' => 'test', 'payment_method' => 'cash', 'balance' => 0]);

        $this->backfillBalances();

        $journals = Journal::orderBy('date', 'asc')->get();
    ...
    {
        $currentBalance = 0;
        $journals = Journal::orderBy('date', 'asc')->orderBy('id', 'asc')->get();
