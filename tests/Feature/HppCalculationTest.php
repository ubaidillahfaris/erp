...
class HppCalculationTest extends TestCase
{
    ...
    public function test_hpp_calculates_correctly_for_simple_bom()
    {
        ...
        $gr = Satuan::create(['nama' => 'Gram', 'simbol' => 'gr']);

        // $kg->conversions()->create(['to_satuan_id' => $gr->id, 'rasio' => 1000]);
        // Inverse for the test: $gr to $kg (1 gr = 0.001 kg)
        $gr->conversions()->create(['to_satuan_id' => $kg->id, 'rasio' => 0.001]);

        // Raw Material: Tepung (10000 / kg)
        $tepung = Produk::create([
            'nama' => 'Tepung',
            'type' => 'raw_material',
            'satuan_id' => $kg->id,
        ]);
        ...
        $roti = Produk::create([
            'nama' => 'Roti',
            'type' => 'finished_good',
            'satuan_id' => Satuan::create(['nama' => 'Pcs', 'simbol' => 'pcs'])->id,
        ]);

        $bom = Bom::create(['produk_id' => $roti->id, 'sku' => 'BOM-ROTI']);
        $bom->items()->create([
            'produk_id' => $tepung->id,
            'satuan_id' => $gr->id,
            'jumlah' => 200, // 200gr = 0.2kg
        ]);
    ...
    public function test_hpp_updates_recursively_for_nested_bom()
    {
        ...
        $tepung->prices()->create(['satuan_id' => $kg->id, 'purchase_price' => 10000, 'is_current' => true]);

        // Semi-finished: Adonan (Uses 1kg Tepung)
        $adonan = Produk::create(['nama' => 'Adonan', 'type' => 'semi_finished', 'satuan_id' => $kg->id]);
        ...
        $roti = Produk::create(['nama' => 'Roti', 'type' => 'finished_good', 'satuan_id' => Satuan::create(['nama' => 'Pcs', 'simbol' => 'pcs'])->id]);
    ...
    public function test_retail_price_is_preserved_during_hpp_recalculation()
    {
        ...
        $roti = Produk::create(['nama' => 'Roti', 'type' => 'finished_good', 'satuan_id' => $kg->id]);
        $roti->prices()->create([
            'satuan_id' => $kg->id,
            'purchase_price' => 5000,
