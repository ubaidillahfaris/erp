    ...
    <div class="w-full lg:w-[400px] bg-white flex flex-col shadow-2xl shadow-slate-200/50 relative z-10">
        ...
        <div class="flex-1 overflow-y-auto p-6 scrollbar-hide">
            <div class="flex items-center justify-between mb-4">
                ...
                <Badge variant="secondary" class="rounded-full bg-slate-100 text-slate-500 font-bold border-none">{{ items.length }} Items</Badge>
            </div>
            ...
            <div class="space-y-3">
                ...
                >
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[11px] text-slate-400 font-medium">{{ formatCurrency(item.price) }} / {{ item.unit_symbol }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <button @click="emit('decrement', item.product_id)" class="h-7 w-7 rounded-lg border border-slate-100 flex items-center justify-center hover:bg-slate-50 active:scale-90 transition-all">
                        ...
                        <button @click="emit('increment', item.product_id)" class="h-7 w-7 rounded-lg bg-primary/5 flex items-center justify-center hover:bg-primary/10 active:scale-90 transition-all">
        ...
        <div class="p-6 border-t border-slate-100 bg-white">
            ...
            <p class="text-[10px] text-center text-slate-400 mt-4 font-bold uppercase tracking-widest">Syarat & Ketentuan Berlaku</p>
        </div>
    </div>
</template>
