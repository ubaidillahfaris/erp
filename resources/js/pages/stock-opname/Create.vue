        ...
        <div class="flex flex-col gap-10">
            ...
            <section class="space-y-6">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
                    <h3 class="text-xs font-bold uppercase tracking-widest text-muted-foreground">2. Daftar Barang
                    </h3>
                    ...
                    <Table class="rounded-none border-none">
                        <TableHeader>
                            <TableRow class="hover:bg-transparent border-b border-muted">
                                <TableHead
                                    class="h-12 px-0 text-xs font-bold uppercase tracking-wider text-muted-foreground">
                                    Barang</TableHead>
                                <TableHead
                                    class="h-12 px-0 text-right text-xs font-bold uppercase tracking-wider text-muted-foreground">
                                    Stok Sistem</TableHead>
                                <TableHead
                                    class="h-12 px-0 w-[160px] text-right text-xs font-bold uppercase tracking-wider text-muted-foreground">
                        ...
                        <TableBody>
                            <TableRow v-for="produk in produks.data" :key="produk.id"
                                ...
                                <TableCell class="px-0 py-4">
                                    ...
                                    <div
                                        class="text-xs font-mono text-muted-foreground uppercase tracking-tighter mt-0.5">
                                        {{ produk.sku }}</div>
                                </TableCell>
                                <TableCell class="px-0 py-4 text-right text-sm font-medium">
                                    {{ form.items_map[produk.id]?.system_qty }} <span
                                        class="text-xs text-muted-foreground uppercase">{{ produk.satuan?.nama
                                        }}</span>
                                </TableCell>
                                <TableCell class="px-0 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <span
                                            class="text-xs font-bold text-muted-foreground w-8 text-left uppercase">{{
