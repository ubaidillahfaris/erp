...
const submit = async (status: 'draft' | 'completed') => {
    ...
    if (await confirmDialog(msgTitle, msg)) {
        ...
        form.transform((data) => ({
            ...data,
            items: Object.values(data.items_map) })).put(`/stock-opname/${props.opname.id}`);
    }
};
    ...
    <div class="px-6 py-8 bg-slate-50 min-h-[calc(100vh-64px)] flex flex-col gap-6 font-sans">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold tracking-tight text-slate-900">Edit Draft Stock Opname</h1>
                    <p class="text-sm text-slate-400 mt-0.5">Lanjutkan menginput stok fisik atau ubah data draft.</p>
                </div>
            </div>
