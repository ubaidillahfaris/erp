...
const form = useForm({
    ...
    conversions: props.unit.conversions.map(c => ({
        target_unit_id: c.to_unit_id.toString(),
        ratio: Number(c.rasio)
    })) || []
});

let oldNama = props.unit.name;

watch(() => form.name, (newNama) => {
    if (form.symbol === oldNama.substring(0, 3).toLowerCase()) {
...
const submit = () => {
    form.put(satuanUpdate(props.unit.id).url);
};
</script>

<template>
<Head title="Edit Product Units" />
...
<AppLayout :breadcrumbs="breadcrumbs">
    <div class="px-6 py-8 bg-slate-50 min-h-[calc(100vh-64px)] flex flex-col gap-6 font-sans">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl font-bold tracking-tight text-slate-900">Edit Product Units</h1>
        ...
        <Card class="border border-slate-200 rounded-xl bg-white shadow-none">
            <div class="px-6 py-4 border-b border-slate-100">
                <h3 class="text-sm font-semibold text-slate-900 leading-none">Ubah Informasi Unit</h3>
