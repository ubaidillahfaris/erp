...
watch([view, search, date_start, date_end, status, perPage, sort, direction], debouncedFilter);
...
<div class="px-6 py-8 bg-slate-50 min-h-[calc(100vh-64px)] flex flex-col gap-6 font-sans">
    ...
    <PageHeader 
        ...
        description="Pantau dan kelola seluruh alur kerja servis" 
        back-href="/dashboard" 
        :count="orders.total" 
    >
        <template #actions>
    ...
    <div class="w-full max-w-7xl mx-auto flex-1 flex flex-col min-h-0">
            ...
            <template #actions="{ row }">
                <div class="flex items-center gap-1">
                    <Button 
                        variant="ghost" 
                        size="icon" 
                        class="h-8 w-8 text-orange-500 hover:text-orange-600 hover:bg-orange-50"
        ...
        <div v-else class="flex flex-col h-full min-h-[600px]">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-3">
                    <Button variant="outline" class="h-10 rounded-xl border-slate-200 bg-white shadow-sm hover:bg-slate-50 gap-2 px-4" @click="showAddStepModal = true">
            ...
            <div class="flex-1 overflow-x-auto overflow-y-hidden flex gap-6 pb-4 custom-scrollbar">
                ...
                <div v-for="step in steps" :key="step.id" class="w-80 shrink-0 flex flex-col gap-4">
                    ...
                    <!-- Column Body -->
                    <draggable 
                        v-model="localOrdersByStep[step.id]" 
                        group="orders" 
                        @change="onDragChange($event, step.id)"
                        item-key="id" 
                        class="flex-1 overflow-y-auto space-y-4 pr-1 custom-scrollbar pb-10"
                                ...
                                <div class="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-1">
                                        <Button 
                                            ...
                                            title="Sesuaikan Harga"
                                            @click.stop="openPriceModal(order)"
                                        >
            ...
            <div class="mt-4 py-3 border-t border-slate-200 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <Button 
                        variant="outline" size="sm" class="h-8 rounded-lg text-[10px] font-bold uppercase tracking-tighter" 
                        :disabled="!orders.links[0]?.url" 
                        @click="router.visit(orders.links[0].url)"
                    >
                        Previous
                    </Button>
                    <Button 
    ...
    <Dialog v-model:open="priceOpen">
        <DialogContent class="max-w-sm rounded-2xl border-slate-200 shadow-2xl">
            ...
            <div class="py-6 space-y-4">
                <div class="space-y-2">
                    <Label class="text-[10px] font-bold uppercase tracking-widest text-slate-400">Total Biaya (IDR)</Label>
                ...
                <div class="p-3 rounded-xl bg-orange-50 border border-orange-100 flex gap-3">
                    ...
                    <p class="text-[10px] font-medium text-orange-700 leading-relaxed">
                        Pastikan Anda sudah mengonfirmasi perubahan harga ini kepada pelanggan. Perubahan ini akan langsung memperbarui total tagihan.
                    </p>
                </div>
            </div>
